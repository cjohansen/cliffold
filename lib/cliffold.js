...
function formatHelp(options) {
    var str = "";
    if (options.help.name) { str += options.help.name + "\n\n"; }
    if (options.help.synopsis) { str += options.help.synopsis + "\n\n"; }
    var formatted = [], used = [];
...
function tryHelp(stdout, options, values) {
    ...
    stdout.write(formatHelp(options));
    return true;
}
...
Cli.prototype = {
    ...
    formatHelp: function () {
        return formatHelp(this.options);
    },
