...
function logHelp(stdout, options, name, synopsis) {
    if (name) { stdout.write(name + "\n\n"); }
    if (synopsis) { stdout.write(synopsis + "\n\n"); }
    var formatted = [], used = [];
    ...
    stdout.write(formatted.map(function (opt) {
        return "    " + opt[0] + "\n" + formatDesc(opt[1]) + "\n";
    }).join("\n"));
}
...
function tryHelp(stdout, options, values) {
    ...
    logHelp(stdout, options, options.help.name, options.help.synopsis);
    return true;
}
